...
import pl.webd.dawid124.ioengine.home.devices.driver.instance.EIoDriverType;
import pl.webd.dawid124.ioengine.home.devices.output.IDevice;
import pl.webd.dawid124.ioengine.home.devices.output.LedDevice;
    ...
    private void divideBrightnessByGroupValue(String zoneId, String sceneId, IoAction action, List<LightGroup> groups) {
        getGroupByIoId(groups, action.getIoId()).ifPresent(g -> {
            ...
            if (ledDeviceState.getBrightness() < MAX_LED_VALUE) {
                double percent = (double) MAX_LED_VALUE / (double) ledDeviceState.getBrightness();
